...
from shatoon.commands import DiskFree, MeasureTemperature, MeasureVoltage
from shatoon.runners import BaseCommand
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "volt=0.7V",
            "Measure voltage incorrect: 0.7. Should be of 0.8V to 1.35V",
            False,
        ),
        (
            "volt=1.419V",
            "Measure voltage incorrect: 1.419. Should be of 0.8V to 1.35V",
            False,
        ),
...
@pytest.mark.asyncio
@patch("shatoon.commands.BaseCommand.process_command")
@pytest.mark.parametrize("command_class", [BaseCommand, MeasureTemperature, MeasureVoltage, DiskFree])
...
async def test_command_run_with_exception(process_command_mock: AsyncMock, command_class):
    ...
    await MockedCommand.run()
    assert MockedCommand.error_message == f"Can't process or validate command: {exc!s}"
